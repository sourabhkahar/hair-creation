...
function setRecords() {
    ...
    documentSnapshots.forEach((doc) => {
        artistsData.value.push({ ...doc.data(), id: doc.id });
    });
}
async function getNewReords() {
    await getNextReords(lastVisible)
    page++;
    setRecords()
}
...
async function getArtistsList(srcTxt = '') {
    await loadRecords(srcTxt)
    totalRecs.value = await totalArtists()
    setRecords()
}
const diabledNextBtn = computed(() => {
    return artistsData.value.length < perPage || (page * perPage) >= totalRecs.value
})
const search = useDebounceFn(async () => {
    await getArtistsList(searhText.value);
}, 1000)
...
async function deleteRecord() {
    ...
    const res = await updateArtists(selectedId.value, payLoad)
    if (res) {
...
<template>
    ...
    <div class="flex overflow-hidden bg-white pt-16">
        ...
        <div id="main-content" class="h-full w-full bg-gray-50 relative overflow-y-auto lg:ml-64">
                        ...
                        <div class="bg-white rounded-xl shadow-md overflow-hidden">
                            ...
                            <div class="p-6 border-b border-gray-200">
                                <div class="flex flex-col md:flex-row md:items-center md:justify-between">
                                    <div>
                                        <h2 class="text-xl font-bold text-gray-800">Add Artists
                                        </h2>
                                    </div>
                            ...
                            <div class="overflow-x-auto">
                                <table class="min-w-full divide-y divide-gray-200">
                                    ...
                                    <tbody class="bg-white divide-y divide-gray-200">
                                        <!-- Row 1 -->
                                        <template v-for="(artist, index) in artistsData" :key="index">
                                            <tr class="hover:bg-gray-50 transition-colors duration-150">
                                                ...
                                                <td class="px-6 py-4 whitespace-nowrap">
                                                    <div class="text-sm text-gray-900">{{ artist.designation }}
                                                    </div>
                                                </td>
                                                <td class="px-6 py-4 whitespace-nowrap">
                                                    <div class="text-sm text-gray-900">
                                                        {{ formatDate(artist.created_at) }}</div>
                                                </td>
                                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                                    <RouterLink :to="`artists/${artist.id}/edit/`"
                                                        class="text-indigo-600 hover:text-indigo-900 mr-3">Edit
                                                    </RouterLink>
                                                    <a href="#" @click.prevent="toggleDeleteModal(artist.id)"
                                                        class="text-red-600 hover:text-red-900">Delete</a>
                                                    <RouterLink :to="{name:'artists-services',params:{id:artist.id}}"
                                                        class="text-indigo-600 hover:text-indigo-900 ml-3">Services
                                                    </RouterLink>
                                                </td>
                                            </tr>
                                        </template>
                                ...
                                <div class="flex items-center justify-between flex-col sm:flex-row">
                                    <div class="mb-4 sm:mb-0">
                                        <p class="text-sm text-gray-700">
                                            Showing <span class="font-medium">{{ artistsData.length }}</span> of
            ...
            <!-- Overlay -->
            <div id="deleteModal" class="fixed inset-0 z-50 flex  items-center justify-center bg-black/50" v-if="showDeleteModal">
                <!-- Modal -->
